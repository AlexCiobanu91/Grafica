...
namespace lab{
	...
	void loadObj(GLuint gl_program_shader, const std::string &filename, unsigned int &vao, unsigned int& vbo, unsigned int &ibo, unsigned int &num_indices, int stare,
                std::vector<VertexFormat> &verticesRet, std::vector<unsigned int> &indicesRet){
	
		//incarca vertecsii si indecsii din fisier
		std::vector<VertexFormat> vertices;
		std::vector<unsigned int> indices;
		_loadObjFile(filename, vertices, indices);
    verticesRet = vertices;
    indicesRet = indices;
		...
		switch (stare) {
			case 1:
				pipe = glGetAttribLocation(gl_program_shader, "in_position_Asleep");
				glEnableVertexAttribArray(pipe);
				glVertexAttribPointer(pipe,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)0);				     
				
				pipe = glGetAttribLocation(gl_program_shader, "in_normals_Asleep");
				glEnableVertexAttribArray(pipe);
				glVertexAttribPointer(pipe,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(sizeof(float)*3));		

				pipe = glGetAttribLocation(gl_program_shader, "in_texcoord_Asleep");
				glEnableVertexAttribArray(pipe);
				glVertexAttribPointer(pipe, 2,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(2*sizeof(float)*3));		
			break;
			case 2:
				pipe = glGetAttribLocation(gl_program_shader, "in_position_Annoyed");
				glEnableVertexAttribArray(pipe);
				glVertexAttribPointer(pipe, 3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)0);				     
			...
			break;
			case 3:
				pipe = glGetAttribLocation(gl_program_shader, "in_position_Surprised");
				glEnableVertexAttribArray(pipe);
				glVertexAttribPointer(pipe, 3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)0);				     
		...
		//int pipe2 = glGetAttribLocation(gl_program_shader, "in_color");
		//glEnableVertexAttribArray(pipe2);
		//glVertexAttribPointer(pipe2,3,GL_FLOAT,GL_FALSE,sizeof(VertexFormat),(void*)(sizeof(float)*3));		//trimite normale pe pipe 1
